...
template <class InputType, class ConstraintType>
struct Constraint {
...
struct IsEqual : public Constraint<DataType, DataType> {
  static_assert(
    true == equality<DataType>::value,
    "'DataType' must support equality operator!"
    );
  IsEqual(const DataType & constraint)
  ...
  std::string str() override {
    return std::to_string(Constraint<DataType, DataType>::constraint_value);
  }
};
